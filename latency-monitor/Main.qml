...
Item {
    ...
    readonly property var worstHost: {
        const order = { "critical": 3, "warning": 2, "good": 1, "unknown": 0 }
        let worst = null
    ...
    readonly property var displayHost: {
        if (barHost === "worst" || !barHost) return worstHost
    ...
    function _rebuildHosts() {
        for (const h of hosts) h.destroy()
        const built = []
    ...
    onThresholdGoodChanged: {
        for (const h of hosts) h.thresholdGood = thresholdGood
    }
    onThresholdWarningChanged: {
        for (const h of hosts) h.thresholdWarning = thresholdWarning
    }
